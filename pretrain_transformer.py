...
PATH_TO_VAL_IMAGES = "Dataset/Validation/"
...
if __name__ == '__main__':
    ...
    parser.add_argument('--log_output', type=str,   # added logging arg 
    ...
    train_data, train_labels = load_data_and_labels(PATH_TO_TRAIN_IMAGES, 15000, device) # 15000
    ...
    val_data, val_labels = load_data_and_labels(PATH_TO_VAL_IMAGES, 3750, device) # 3750
    ...
    model = nn.Sequential(
    ViT,
    nn.Linear(1000,256),  # changed hidden size
    nn.ReLU(),
    ...
    _, best_model_state_dict = train(model, optimizer, 1, train_dataloader, val_dataloader, device=device, epochs=10)
    model.load_state_dict(best_model_state_dict)

    # evaluate on test set
    test_data, test_labels = load_data_and_labels(PATH_TO_TEST_IMAGES, 3750, device) #3750
